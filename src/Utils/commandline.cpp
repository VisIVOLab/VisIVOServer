...
//---------------------------------------------------------------------
{
  ...
  for (i = 0; i < arguments.size(); i++) {
    if (arguments[i] == "--missingvalue") {
      /*      		  std::string ckInput=arguments[i+1];
                        if(ckInput.rfind('--')==0)
                        {
                          std::cerr<<"Error on "<<arguments[i]<< "argument:
         "<<ckInput<<std::endl; return -1;
                        }	*/
      ...
      if (i == arguments.size() - 2) {
        if (m_type != "hecuba") {
          std::cerr
              << "Output file and input file cannot be have the same filename"
              << std::endl;
          return -1;
        }
      }
  ...
  m_currentPath =
      arguments[(arguments.size() - 1)]; //! filename including path!!
  // check for gLite lfn
  ...
  if (m_currentPath.substr(0, 7) == "http://" ||
      m_currentPath.substr(0, 7) == "sftp://" ||
      m_currentPath.substr(0, 6) == "ftp://") {
    bool remoteOp;
    std::string downloadedPath, remoteFile;
    remoteFile = m_currentPath;
    if (m_out != "nopath")
      downloadedPath = getDir(m_out);
    ...
    if ((m_type == "binary" && m_binaryHeader == "noheader") ||
        (m_type == "binary" && (m_binaryHeader.substr(0, 7) == "http://" ||
                                m_binaryHeader.substr(0, 7) == "sftp://" ||
                                m_binaryHeader.substr(0, 6) == "ftp://"))) {
  ...
  if (m_binaryHeader.substr(0, 7) == "http://" ||
      m_binaryHeader.substr(0, 7) == "sftp://" ||
      m_binaryHeader.substr(0, 6) == "ftp://") {
    bool remoteOp;
    std::string headerFile, downloadedHeader;
  ...
  //
  if (m_file == "volume")
    if ((m_comput[0] <= 0 || m_comput[1] <= 0 || m_comput[2] <= 0) &&
        (m_type != "binary")) {
      if (m_type != "hdf5" && m_type != "fitsimage") {
        std::cerr << "A volume must  have valid x y and z mesh point dimensions"
                  << std::endl;
        return -1;
      }
    }
  ...
  if (m_type != "fitstable" && m_type != "ramses" && m_type != "fitsimage" &&
    ...
    if (!inFile) {
      std::cerr
          << "the path is incorrect, please try again  or --help for help \n";
      return -1;
    }
...
*/
    ...
    /*
      ...
      std::cerr << "the format given '" << m_type
                << "' is incorrect, please try again  or --help for help"
                << std::endl;
      return -1;
    }
    pSource->setUseMem(m_inMemory);
    pSource->setPointsFileName(
        m_currentPath.c_str(), m_binaryPath.c_str(), m_file.c_str(), m_size,
    ...
    if (pSource->readHeader() == 0)
      pSource->readData();
    if (m_historyEnabled) {
      pSource->writeHistory(
          m_historyFile.c_str(), m_type.c_str(), m_out.c_str(), m_file.c_str(),
          m_comput, m_size, m_login.c_str(), m_binaryHeader.c_str(), m_missing,
          m_text, m_endian.c_str(), m_dataType.c_str(), m_npoints, m_VO.c_str(),
          m_se.c_str(), m_outlfn.c_str(), m_currentPath.c_str());
    }
...

{
  ...
  std::cout << std::endl;
  std::cout << "VisIVOImporter Version 2.1.1 June 28th 2013 " << std::endl
            << std::endl;

  std::cout << " --fformat   [typefile]  (mandatory) Select file type: ascii, "
               "csv, votable, binary, fly, gadget, xml, rawpoints, rawgrids, "
               "fitstable, fitsimage, hdf5, muportal, ramses"
            << std::endl
            << std::endl;

  std::cout << "[pathfile] (mandatory) Absolute path file. Path must be the "
               "last command( /home/user/myfile.ascii)"
            << std::endl
            << std::endl;

  std::cout << "--out [filename]   (optional) Change default file name  and or "
               "directory ( --out /home/user/myfile.bin )  "
            << std::endl
            << std::endl;

  std::cout
      << "--volume  (optional)  if you want a table for volume.Is mandatory if "
         "you want select cell size and/or computational cell size "
      << std::endl
      << std::endl;

  std::cout << "--sizex [double] --sizey [double]  --sizez [double]  "
               "(optional)  if you want  select cell size(--sizex 1 --sizey 1  "
               "--sizez 1 ). If you use this command --volume is mandatory .If "
               "don't use this commands default size is 1 1 1 "
            << std::endl
            << std::endl;

  std::cout << "--compx [double] --compy [double] --compz [double]  (optional) "
               " if you want  select computational cell size. If the "
               "mathematical product of this tree values is different from "
               "field size the created output will be a table."
            << std::endl
            << std::endl;

  std::cout << "--bigendian (optiona//l) use this command only if 'gadget' and "
               "'fly' format file are big endian"
            << std::endl
            << std::endl;

  std::cout << "--double (optional) use this command only if the 'fly' format "
               "file has double data type"
            << std::endl
            << std::endl;

  std::cout << "--npoints (optional) use this command only to set the number "
               "of points in the 'fly' format file "
            << std::endl
            << std::endl;

  std::cout
      << "--datasetlist  use this command only to select datasets in hdf5 file"
      << std::endl
      << std::endl;

  std::cout << "--hyperslab  use this command only to select dataset hyperslab "
               "in hdf5 file"
            << std::endl
            << std::endl;

  std::cout << "--history (optional) create an XML file which contains the "
               "history of operations performed (default create hist.xml file)"
            << std::endl
            << std::endl;

  std::cout << "--historyfile [filename]   (optional) Change default history "
               "file name  and or directory "
            << std::endl
            << std::endl;
}

std::vector<VSTable *> &
CommandLine::getTable() { // return pSource->getMemTables();
